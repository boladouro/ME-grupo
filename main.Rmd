...
pessoa <- trajectory("pessoa") %>%
  ...
  renege_in(\() ifelse(get_attribute(env, "tipo_de_consulta") == tipo_de_consulta.TELEFONICA, 5, Inf), out = 
              ...
              rollback(target="contacto") # se nao funcionar mudar pra numero
            ) %>%
  visit("server", \() rnorm(1, mean = 4, sd = 1) + rnorm(1, mean = 1, sd = 0.25) + ifelse(get_attribute(env,"tipo_de_consulta") != tipo_de_consulta.NAO_AGENDADA, rnorm(1, mean = 1, sd = 0.25), 0))
pessoa
```
...
env %>% 
  ...
  add_generator("pessoa", pessoa, \() rexp(1, 1/40)) %>% #TODO ver isto
  run(10)
