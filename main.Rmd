...
arrivals %>% 
  ...
  mutate(tipo_de_contacto = ifelse(value == tipo_de_contacto.TELEFONE, "telefone", "mensagem") %>% as_factor) %>% 
  select(name, replication, waiting_time, tipo_de_contacto) -> waiting_times

# histograma
waiting_times %>% 
  ggplot(aes(x = waiting_time, after_stat(density))) +
  geom_histogram(aes(fill = tipo_de_contacto), position = "identity", alpha = 0.5, bins = 100) +
  facet_wrap(vars(tipo_de_contacto)) +
  theme(legend.position = "none")
waiting_times %>% 
  ggplot(aes(x = waiting_time, after_stat(density))) +
  geom_histogram(aes(fill = tipo_de_contacto), position = "identity", alpha = 0.5, bins = 100) +
